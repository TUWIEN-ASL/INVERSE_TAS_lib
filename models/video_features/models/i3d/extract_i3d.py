...
import torch
import torchvision
from models.video_features.models._base.base_extractor import BaseExtractor
from models.video_features.models.i3d.i3d_src.i3d_net import I3D
from models.video_features.models.raft.extract_raft import DATASET_to_RAFT_CKPT_PATHS
from models.video_features.models.raft.raft_src.raft import RAFT, InputPadder
from models.video_features.models.transforms import (Clamp, PermuteAndUnsqueeze, PILToTensor,
                               ResizeImproved, ScaleTo1_1, TensorCenterCrop,
                               ToFloat, ToUInt8)
from models.video_features.utils.io import reencode_video_with_diff_fps
from models.video_features.utils.utils import dp_state_to_normal, show_predictions_on_dataset
...
class ExtractI3D(BaseExtractor):
    ...
    def extract(self, video_path: str) -> Dict[str, np.ndarray]:
        ...
        total_stacks = max(0, (total_frames - self.stack_size) // self.step_size + 1)
        ...
        padder = None
        stack_counter = 0
        pbar = tqdm(total=total_stacks, desc=Path(video_path).name, unit='stack', leave=False)
        while cap.isOpened():
            ...
            if frame_exists:
                ...
                if len(rgb_stack) - 1 == self.stack_size:
                    # try:
                    #     e_collect = time.process_time_ns()
                    #     print("collection time", (e_collect - s_collect)/10**9)
                    # except:
                    #     pass
                    # s_inf = time.process_time_ns()
                    batch_feats_dict = self.run_on_a_stack(rgb_stack, stack_counter, padder)
                    # e_inf = time.process_time_ns()
                    # print("Inference time", (e_inf - s_inf)/10**9)
                    ...
                    pbar.update(1)
                    # s_collect = time.process_time_ns()
            else:
        ...
        for stream in self.streams:
            ...
            if stream == 'flow':
                if self.flow_type == 'raft':
                    # flow_s = time.process_time_ns()
                    stream_slice = flow_xtr_model(padder.pad(rgb_stack)[:-1], padder.pad(rgb_stack)[1:])
                    # flow_e = time.process_time_ns()
                    # print(f"\t Flow time {stream}", (flow_e - flow_s)/10**9)
                else:
                    raise NotImplementedError
            elif stream == 'rgb':
            ...
            # import pdb
            # pdb.set_trace()
            # trans_s = time.process_time_ns()
            stream_slice = self.i3d_transforms[stream](stream_slice)
            # trans_e = time.process_time_ns()
            # print(f"\t Trans time {stream}", (trans_e - trans_s)/10**9)
            # extract features for a stream
            # import pdb
            # pdb.set_trace()
            # inf_s = time.process_time_ns()
            batch_feats_dict[stream] = models[stream](stream_slice, features=True)  # (B, 1024)
            # inf_e = time.process_time_ns()
            # print(f"\t Inf time {stream}", (inf_e - inf_s)/10**9)
    ...
    def load_model(self) -> Dict[str, torch.nn.Module]:
        ...
        i3d_weights_paths = {
            'rgb':  f'{project_path}/models/video_features/models/i3d/checkpoints/i3d_rgb.pt',
            'flow': f'{project_path}/models/video_features/models/i3d/checkpoints/i3d_flow.pt',
        }
        name2module = {}
