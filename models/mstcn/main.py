...
if args.action == "train":
    ...
    run = init_wandb("mstcn", args.dataset, args.split, args.f_type,
                     extra_config=dict(num_stages=num_stages, num_layers=num_layers,
                                       num_f_maps=num_f_maps, features_dim=features_dim,
                                       num_classes=num_classes, lr=lr, num_epochs=num_epochs))
...
if args.action == "predict":
    ...
    for i in range(num_epochs):
        epoch_path =  results_dir + f"/epoch_{i+1}"
        if not os.path.exists(epoch_path):
            os.makedirs(epoch_path)
        trainer.predict(model_dir, epoch_path, features_path, vid_list_file_tst, i+1, actions_dict, device, sample_rate)
