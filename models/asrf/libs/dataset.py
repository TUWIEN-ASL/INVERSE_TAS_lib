...
class ActionSegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        csv_dir: str = f"{project_path}/models/asrf/csv",
    ) -> None:
        super().__init__()
        """
            Args:
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        oaf_path = self.df.iloc[idx]["oaf"]

        feature = np.load(feature_path).astype(np.float32)
        label = np.load(label_path).astype(np.int64) #[1:]
        boundary = np.load(boundary_path).astype(np.float32) #[1:]


        # if not np.isnan(oaf_path):
        oaf_feature = np.load(oaf_path).astype(np.float32).T
        min_size = min(feature.shape[-1], oaf_feature.shape[-1])
        feature = feature[:, :min_size]
        oaf_feature = oaf_feature[:, :min_size]
        feature = np.concatenate([feature, oaf_feature], 0)

        min_size = min(feature.shape[-1], label.shape[0], boundary.shape[0])
        feature = feature[:, :min_size]
