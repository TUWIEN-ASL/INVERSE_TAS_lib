...
from models.asrf.libs.class_id_map import get_id2class_map
from models.asrf.libs.metric import AverageMeter, BoundaryScoreMeter, ScoreMeter
...
def validate(
    ...
    with torch.no_grad():
        for sample in val_loader:
            ...
            # import pdb; pdb.set_trace()
            refined_output_cls = postprocessor(output_cls, boundaries=output_bound, masks=mask)
    ...
    cls_acc, edit_score, segment_f1s, bound_f1s =  scores_after_refinement.get_scores()
...
def evaluate(
    ...
    with torch.no_grad():
        for sample in val_loader:
            ...
            # import pdb; pdb.set_trace()
            mapping_file_path = "/home/dsliwowski/Projects/Code_Inverse/catkin/src/inverse_tas/data/ImPerfectPour/mapping.txt"
            mapping_file = np.loadtxt(mapping_file_path, dtype=str)

            actions_dict = dict()
            for a in mapping_file:
                actions_dict[a[1]] = int(a[0])

            predicted = refined_output_cls.squeeze()

            recognition = []
            for i in range(len(predicted)):
                recognition = np.concatenate((recognition, [list(actions_dict.keys())[list(actions_dict.values()).index(predicted[i].item())]]))
            
            rec_name = os.path.split(sample["feature_path"][0])[-1]
            f_name = os.path.splitext(rec_name)[0] + ".txt"
            results_dir = os.path.join(result_path, "prediction")
