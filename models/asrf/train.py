...
def main() -> None:
    ...
    result_path = os.path.join(project_path, "checkpoints", "ASRF", str(args.epoch))
    ...
    wandb_run = init_wandb("asrf", config.dataset, config.split, f"in_channel-{config.in_channel}",
                           extra_config=dict(epoch=args.epoch, in_channel=config.in_channel,
                                             n_features=config.n_features, n_classes=n_classes,
                                             n_stages=config.n_stages, n_layers=config.n_layers,
    ...
    for epoch in range(begin_epoch, config.max_epoch):
        ...
        if best_f150 < segment_f1s[-1]:
            best_f150 = segment_f1s[-1]
            torch.save(
                model.state_dict(),
                os.path.join(result_path, "best_f150_model.prm"),
            )

        print("VAL loss, acc, edit, f1s", val_loss, cls_acc, edit_score, segment_f1s)
        wandb_run.log({
            "train/loss": train_loss,
            "val/loss": val_loss,
            "val/acc": cls_acc,
            "val/edit": edit_score,
            "val/f1_10": segment_f1s[0] if len(segment_f1s) > 0 else 0,
            "val/f1_25": segment_f1s[1] if len(segment_f1s) > 1 else 0,
            "val/f1_50": segment_f1s[2] if len(segment_f1s) > 2 else 0,
        }, step=epoch)
