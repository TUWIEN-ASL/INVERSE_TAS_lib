...
class ASDiffusionModel(nn.Module):
    ...
    def get_training_loss(self, video_feats, event_gt, boundary_gt,
        ...
        # boundary_slice = boundary_gt[:,:,1:]

        # print("=== Boundary GT Debug ===")
        # print(f"Shape: {boundary_slice.shape}")
        # print(f"Dtype: {boundary_slice.dtype}")
        # print(f"Min: {boundary_slice.min().item()}")
        # print(f"Max: {boundary_slice.max().item()}")
        # print(f"Unique values: {torch.unique(boundary_slice)}")
        # print(f"All zeros: {(boundary_slice == 0).all()}")
        # print(f"Any negative: {(boundary_slice < 0).any()}")
        # print(f"Any > 1: {(boundary_slice > 1).any()}")
        # print(f"Any NaN: {torch.isnan(boundary_slice).any()}")
        # print(f"Any Inf: {torch.isinf(boundary_slice).any()}")
...
from models.DiffAct.dataset import restore_full_sequence
from models.DiffAct.utils import mode_filter, get_labels_start_end_time
   
class Segmentator:
    def __init__(self, model_path, encoder_params, decoder_params, diffusion_params, 
        num_classes, sample_rate, temporal_aug, set_sampling_seed, postprocess, device):
