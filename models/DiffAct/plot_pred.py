...
from models.DiffAct.utils import load_config_file, func_eval, set_random_seed, get_labels_start_end_time, plot_barcode
from models.DiffAct.utils import mode_filter
...
class Trainer:
    ...
    def train(self, train_train_dataset, train_test_dataset, test_test_dataset, loss_weights, class_weighting, soft_label,
        ...
        if class_weighting:
            ...
            ce_criterion = nn.CrossEntropyLoss(
                ignore_index=-100, weight=class_weights, reduction='none')
        else:
            ce_criterion = nn.CrossEntropyLoss(
        ...
        for epoch in range(restore_epoch+1, num_epochs):
            ...
            for id_, data in enumerate(train_train_loader):

                feature, label, boundary, video = data
                feature, label, boundary = feature.to(
                    device), label.to(device), boundary.to(device)

                loss_dict = self.model.get_training_loss(feature,
                                                         event_gt=F.one_hot(
                                                             label.long(), num_classes=self.num_classes).permute(0, 2, 1),
                ...
                if result_dir:
                    for k, v in loss_dict.items():
                        logger.add_scalar(
                            f'Train-{k}', loss_weights[k] * v.item() / batch_size, step)
                    logger.add_scalar(
                        'Train-Total', total_loss.item() / batch_size, step)
            ...
            if epoch % log_freq == 0:
                ...
                if result_dir:
                    ...
                    torch.save(self.model.state_dict(),
                               f'{result_dir}/epoch-{epoch}.model')
                    torch.save(state, f'{result_dir}/latest.pt')

                # for mode in ['encoder', 'decoder-noagg', 'decoder-agg']:
                # Default: decoder-agg. The results of decoder-noagg are similar
        ...
        with torch.no_grad():
            ...
            if self.postprocess['type'] == 'median':  # before restoring full sequence
                ...
                for c in range(output.shape[0]):
                    smoothed_output[c] = median_filter(
                        output[c], size=self.postprocess['value'])
                output = smoothed_output / \
                    smoothed_output.sum(0, keepdims=True)
    ...
    def test(self, test_dataset, mode, device, label_dir, result_dir=None, model_path=None, downsample=False):
        ...
        with torch.no_grad():
            ...
            for video_idx in tqdm(range(len(test_dataset))):
                ...
                pred_txt = [self.event_list[int(i)] for i in pred]

                if not os.path.exists(os.path.join(result_dir, 'prediction')):
                    os.makedirs(os.path.join(result_dir, 'prediction'))

                file_name = os.path.join(
                    result_dir, 'prediction', f'{video}.txt')
                file_ptr = open(file_name, 'w')
                file_ptr.write('### Frame level recognition: ###\n')
                file_ptr.write(' '.join(pred_txt))
                file_ptr.close()

                plot_barcode(num_classes, label, pred, False, os.path.join(result_dir, video))
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--use_obj',  action='store_true')
    ...
    result_dir = os.path.join(project_path, "results","DiffAct", f"{all_params['dataset_name']}_{args.f_type}", f"split_{all_params['split_id']}")
    ...
    if args.f_type != "BRPOAF":
        ...
        oaf_feature_dir = os.path.join(root_data_dir, dataset_name, "ipp_oaf_vvb")
    ...
    train_train_dataset = VideoFeatureDataset(
        train_data_dict, num_classes, mode='train')
    train_test_dataset = VideoFeatureDataset(
        train_data_dict, num_classes, mode='test')
    test_test_dataset = VideoFeatureDataset(
        test_data_dict, num_classes, mode='test')
    ...
    mp = weights_dir + f"/{dataset_name}-Trained-S1_{args.f_type}/epoch-{args.epoch}.model"
    # import pdb; pdb.set_trace()

    results = trainer.test(test_test_dataset, 'decoder-agg', trainer.device, label_dir, result_dir=os.path.join(result_dir, naming),
                 model_path=mp)
