...
from models.DiffAct.utils import get_labels_start_end_time
from scipy.ndimage import gaussian_filter1d

def get_data_dict(feature_dir, label_dir, video_list, event_list, sample_rate=4, temporal_aug=True, boundary_smooth=None, downsample=False, oaf_feature_dir=None):
    ...
    for video in tqdm(video_list):
        ...
        feature_file = os.path.join(feature_dir, '{}.npy'.format(v_name))
        event_file = os.path.join(label_dir, '{}.txt'.format(video))

        if oaf_feature_dir is not None:
            oaf_feature_file = os.path.join(oaf_feature_dir, '{}.npy'.format(v_name))
        ...
        # if "2023-11-27-17-00-16_cam2" in video:
        #     import pdb; pdb.set_trace()

        feature = np.load(feature_file, allow_pickle=True)
        if oaf_feature_dir is not None:
            oaf_feature = np.load(oaf_feature_file, allow_pickle=True).T
            min_size = min(feature.shape[-1], oaf_feature.shape[-1])
            feature = feature[:, :min_size]
            oaf_feature = oaf_feature[:, :min_size]
            feature = np.concatenate([feature, oaf_feature])
        missing = len(event_seq_raw) - feature.shape[-1]

        min_size = min(feature.shape[-1], len(event_seq_raw))
        feature = feature[:, :min_size]
        event_seq_raw = event_seq_raw[:min_size]

        missing = len(event_seq_raw) - feature.shape[-1]

        # import pdb; pdb.set_trace()
        assert missing == 1 or missing == 0
        if missing == 1:
            event_seq_raw = event_seq_raw[missing:]
        elif missing == 0:
            event_seq_raw = event_seq_raw[1:]
            feature = feature[:, 1:]
        try:
        ...
        # import pdb; pdb.set_trace()
        # if not(feature.shape[1] == event_seq_raw.shape[0]) or not(feature.shape[1] == boundary_seq_raw.shape[0]):
        #     import pdb; pdb.set_trace()
...
class VideoFeatureDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            if boundary.max() > 0:
                boundary /= boundary.max()  # Only normalize if there are non-zero values
