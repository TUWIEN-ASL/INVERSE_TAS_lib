...
class SingleStageModel(nn.Module):
    ...
    def forward(self, x, q, mask):
        ...
        x_cls = torch.concatenate([x, q_enc], dim=-2)
        ...
        out_const = self.const_dec(x_const) * mask[:, 0:1, :]
...
class Trainer:
    def __init__(self, num_blocks, num_layers, num_f_maps, dim_x, dim_const, dim_enc, q_len, num_classes):
        self.model = MultiStageModel(num_blocks, num_layers, num_f_maps, dim_x, dim_const, dim_enc, q_len, num_classes)
        self.ce = nn.CrossEntropyLoss(ignore_index=-100)
        self.bce = nn.BCEWithLogitsLoss(reduce='none', pos_weight=torch.tensor(50.0))
        ...
        for epoch in range(num_epochs):
            ...
            while batch_gen.has_next():
                ...
                for p_cls, p_const in zip(pred_cls, pred_const):
                    ...
                    # import pdb; pdb.set_trace()
                    const_loss = 2*self.bce(p_const, constraint_target_tensor)
                    ...
                    # import pdb; pdb.set_trace()
                    loss += torch.mean(torch.clamp(self.mse(p_const[:, :, 1:], p_const[:, :, :-1]), min=0, max=16))
