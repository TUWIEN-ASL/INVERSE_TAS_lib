...
class BatchGenerator(object):
    def __init__(self, num_classes, actions_dict, gt_path, features_path, sample_rate, downsample_annot=False):
        self.index = 0
    ...
    def next_batch(self, batch_size, if_warp=False): # if_warp=True is a strong data augmentation. See grid_sampler.py for details.
        ...
        for idx, vid in enumerate(batch):
            ...
            if self.downsample_annot:
                content = content[::2]
            min_len = min(np.shape(features)[1], len(content))
            classes = np.zeros(min_len)
            for i in range(len(classes)):
                classes[i] = self.actions_dict[content[i]]

            # Clip features to annotation length before sampling so both stay aligned
            feature = features[:, :min_len:self.sample_rate]
