...
from models.ASFormer.eval import segment_bars_with_confidence
...
class Trainer:
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, batch_gen_tst=None, wandb_run=None, eval_data=None):
        self.model.train()
        ...
        for epoch in range(num_epochs):
            ...
            batch_gen.reset()
            loss_avg = epoch_loss / len(batch_gen.list_of_examples)
            ...
            if (epoch + 1) % 10 == 0 and batch_gen_tst is not None:
                self.test(batch_gen_tst, epoch, wandb_run=wandb_run)
                torch.save(self.model.state_dict(), save_dir + "/epoch-" + str(epoch + 1) + ".model")
                torch.save(optimizer.state_dict(), save_dir + "/epoch-" + str(epoch + 1) + ".opt")

    def test(self, batch_gen_tst, epoch, wandb_run=None):
        self.model.eval()
        correct = 0
        total = 0
        ...
        if wandb_run is not None:
            wandb_run.log({"val/acc": acc}, step=epoch + 1)
    ...
    def evaluate(self, features_path, vid_list_file, gt_path, actions_dict, sample_rate, device):
        ...
        with torch.no_grad():
            ...
            for vid in vid_list:
                ...
                preds = self.model(input_x, torch.ones(input_x.size(), device=device))
                _, predicted = torch.max(preds.data[-1], 1)
...
class Segmentator:
    def __init__(self, model_path, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate, device):
        self.model = MyTransformer(3, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate)
