...
def func_eval(dataset, recog_path, file_list):
    ground_truth_path = f"{project_path}/data/"+dataset+"/annotations/"
    mapping_file = f"{project_path}/data/"+dataset+"/mapping.txt"
    ...
    for vid in list_of_videos:
        ...
        if dataset != "50salads" and len(recog_content) != len(gt_content):
            recog_content =  [item for item in recog_content for _ in range(2)]
...
def main():
    ...
    if args.split == -1:
        for split in range(0, cnt_split_dict[args.dataset] + 1):
            recog_path = f"{project_path}/results/ASFormer/{args.dataset}_{args.f_type}/split_{split}/"
            file_list = f"{project_path}/data/{args.dataset}/splits/test.split{split}.bundle"
        ...
        recog_path = f"{project_path}/results/ASFormer/{args.dataset}_{args.f_type}/split_{args.split}/"
