...
class VideoGenerator:
    def __init__(self, frames_folder, annotations_file, output_path, fps=30, gt_file=None, start_frame=None, end_frame=None):
        self.frames_folder = Path(frames_folder)
        self.annotations_file = Path(annotations_file)
        self.gt_file = Path(gt_file) if gt_file else None
        self.output_path = Path(output_path)
        self.fps = fps
        self.start_frame = start_frame  # 1-based, inclusive
        self.end_frame = end_frame      # 1-based, inclusive
        self.predictions = []
        self.ground_truth = []
        self.action_colors = {}
    ...
    def load_annotations(self):
        ...
        end_idx = self.end_frame if self.end_frame else total_frames  # end_frame is inclusive, so slice up to end_frame
        ...
        if self.start_frame or self.end_frame:
            print(f"Applying frame interval: frames {start_idx + 1} to {end_idx} (0-indexed: {start_idx}:{end_idx})")
            self.predictions = self.predictions[start_idx:end_idx]
            if self.ground_truth:
                self.ground_truth = self.ground_truth[start_idx:end_idx]
    ...
    def get_frame_files(self):
        """Get sorted list of frame files, filtered to the specified interval"""
        print("Scanning for frame files...")
        
        all_jpg_files = list(self.frames_folder.glob("*.jpg"))
        all_jpg_files.extend(list(self.frames_folder.glob("*.png")))
        ...
        print(f"Found {len(all_jpg_files)} total image files in folder")

        # Apply frame interval filter (1-based, inclusive)
        start_idx = (self.start_frame - 1) if self.start_frame else 0
        end_idx = self.end_frame if self.end_frame else len(all_jpg_files)
        start_idx = max(0, min(start_idx, len(all_jpg_files)))
        end_idx = max(start_idx, min(end_idx, len(all_jpg_files)))

        frame_files = [f for f in all_jpg_files[start_idx:end_idx] if f.exists()]

        print(f"Using {len(frame_files)} frames (interval: {start_idx + 1} to {end_idx})")
        ...
        if len(frame_files) != len(self.predictions) and len(self.predictions) > 0:
            ...
            min_len = min(len(frame_files), len(self.predictions))
    ...
    def create_legend(self, width, height=80):
        ...
        item_height = max(height // rows, 10) if rows * 15 > height else 15
        ...
        for i, action in enumerate(actions_list):
            ...
            cv2.rectangle(legend, (x_start, y_pos - 4), (x_start + 12, y_pos + 4), color, -1)
            display_action = action[:15] + "..." if len(action) > 15 else action
            cv2.putText(legend, display_action, (x_start + 16, y_pos + 3), font, font_scale, (255, 255, 255), thickness)
    ...
    def generate_video(self):
        ...
        # Offset for display: show original frame numbers in the video
        frame_number_offset = (self.start_frame - 1) if self.start_frame else 0
        ...
        for frame_idx, frame_file in enumerate(frame_files):
            ...
            # Show original frame number, not the local index
            original_frame_num = frame_number_offset + frame_idx + 1
            frame_text = f"Frame: {original_frame_num}/{frame_number_offset + len(frame_files)}"
            font = cv2.FONT_HERSHEY_SIMPLEX
            font_scale = 0.4
...
def main():
    ...
    parser.add_argument("--start-frame", type=int, default=None,
                       help="First frame to include, 1-based inclusive (default: 1)")
    parser.add_argument("--end-frame", type=int, default=None,
                       help="Last frame to include, 1-based inclusive (default: last frame)")
    ...
    if args.start_frame is not None and args.start_frame < 1:
        print("Error: --start-frame must be >= 1")
        return
    if args.end_frame is not None and args.end_frame < 1:
        print("Error: --end-frame must be >= 1")
        return
    if args.start_frame and args.end_frame and args.start_frame > args.end_frame:
